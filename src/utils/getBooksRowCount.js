...
const getBooksRowCount = async (ignoredGenres, searchPattern) => {
  let q = `SELECT COUNT(*) as totalCount FROM(
    SELECT DISTINCT b.id from  books b  left join books_genres bg on b.id=bg.book_id   WHERE b.title like ? GROUP BY b.id
  ...
  q += ")) as t";
  const [res] = await db.query(q, [searchPattern, ...ignoredGenres]);
  console.log(res);
